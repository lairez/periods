...
intrinsic ComputeGM(~U, der, L, ~ret : fullbasis := false)
...
intrinsic
    CohomologyBasis( ~U :: Rec, ~basis )
    {}

    lmjac := [U`fromx(LeadingMonomial(p)) : p in GroebnerBasis(U`jac) | Degree(p, U`u) le U`dim];
    ret := &join[ DiffStairs(U`ring ! 1, lmjac, GeneratorsSequence(U`ring), U`deg*k - U`dim ) : k in [1..U`dim-1] ];

    basis := Sort(Setseq(ret));
end intrinsic;
