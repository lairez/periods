...
intrinsic
    ComputeCohomologyBasis( ~U :: Rec )
    {}

    if not assigned U`basis then
        lmjac := [U`fromx(LeadingMonomial(p)) : p in GroebnerBasis(U`jac) | Degree(p, U`u) le U`dim];
        lmjac := &join[ DiffStairs(U`ring ! 1, lmjac, GeneratorsSequence(U`ring), U`deg*k - U`dim ) : k in [1..U`dim-1] ];

        U`basis := Sort(Setseq(lmjac));
    end if;
end intrinsic;

intrinsic
    HomReduceMatrix(~U, ~L, ~mat)
    {}

    ComputeCohomologyBasis(~U);
    HomReduce(~U, ~L, U`r);

    mat := Matrix( CoefficientRing(U`ring), #U`basis, #L,
        [ [ MonomialCoefficient(L[j], U`basis[i]) : j in [1..#L] ] : i in [1..#U`basis] ] );
