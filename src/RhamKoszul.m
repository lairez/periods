...
RKGMfmt := recformat< basis, ebasis, proj, gm, gmcerts, projcerts  >;
intrinsic ComputeGM(~U, der, L, ~ret)
  ...
  ret`projcerts := [ p[2] : p in proj];
  proj := [ U`fromx(p[1]) : p in proj ];
  ...
  while #xmons gt 0 do
    ...
    nf := [ -der*p : p in xmons ];
    HomReduce(~U, ~nf, U`r);
    gm cat:= [ U`fromx(p[1]) : p in nf ];
    gmcerts cat:= [ p[2] : p in nf ];
    if Maximum([Degree(p) : p in gm]) gt (U`dim-1)*U`deg then
      error Error("r_toosmall");
    end if;
    xmons := Setseq(&join[ Seqset(Monomials(U`fromx(p[1]))) : p in nf ] diff Seqset(basis));
  end while;
